...
from db import load_db, add_record
...
tabs = st.tabs([t("TAB_LISTINGS"), t("TAB_DEMANDS"), t("TAB_MATCHING"), t("TAB_IMPACT")])
...
with tabs[0]:
    ...
    with st.form("listing_form", clear_on_submit=True):
        ...
        grade = c8.selectbox(t("GRADE"), ["A", "B", "C"], index=0)
    ...
    df = load_db()
    st.dataframe(df[df["record_type"] == "listing"], use_container_width=True)
...
with tabs[1]:
    ...
    df = load_db()
    st.dataframe(df[df["record_type"] == "demand"], use_container_width=True)
...
with tabs[3]:
    ...
    m = compute_impact(df)
    c1, c2 = st.columns(2)
    c1.metric("Avoided waste (kg)", f"{m['avoided_waste_kg']:.0f}")
    c2.metric("Value saved (XOF)", f"{m['value_saved_xof']:.0f}")
